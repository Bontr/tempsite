...
export const createTerrainGeometry = (data: Float32Array, countLimit: number) => {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const stretchedX = x * (1 + edgeStretch * 0.18);
    const ridgeBand = Math.exp(-((z - 1.5) ** 2) / 0.085);
    ...
    const neutralWhite = 0.80 + outline * 0.18;
    const localLift = personLift * (0.03 + outline * 0.07);
    positions[t] = stretchedX;
    positions[t + 1] = data[o + 1];
    positions[t + 2] = z;
    colors[t] = Math.min(1, neutralWhite + localLift);
    colors[t + 1] = Math.min(1, neutralWhite + localLift);
    colors[t + 2] = Math.min(1, neutralWhite + localLift);
