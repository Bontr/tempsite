...
const initializeScene = async () => {
  ...
  const terrainGeometry = createTerrainGeometry(terrain, Math.round(quality.terrainCount * 0.08));
  ...
  const terrainMaterial = createTerrainMaterial(0.42, 0.92, 1.08);
  ...
  const applyScene = (progress: number, time: number) => {
    ...
    terrainMaterial.uniforms.uOpacity.value = 0.38 * landscapeExit;
